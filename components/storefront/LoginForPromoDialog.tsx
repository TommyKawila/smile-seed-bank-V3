"use client";

import { Loader2, Gift } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
} from "@/components/ui/dialog";
import { Button } from "@/components/ui/button";

interface LoginForPromoDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  message: string;
  promoCode?: string;
  onLogin: () => Promise<void>;
  isLoading?: boolean;
  t?: (th: string, en: string) => string;
}

export function LoginForPromoDialog({
  open,
  onOpenChange,
  message,
  onLogin,
  isLoading = false,
  t = (th) => th,
}: LoginForPromoDialogProps) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md border-emerald-200 bg-gradient-to-b from-emerald-50/80 to-white">
        <div className="flex flex-col items-center text-center space-y-4 py-2">
          <div className="rounded-full bg-emerald-100 p-4">
            <Gift className="h-10 w-10 text-emerald-600" />
          </div>
          <DialogHeader>
            <DialogTitle className="text-lg text-zinc-800">
              {t("รับส่วนลดสมาชิกใหม่ 10%", "Get 10% New Member Discount")}
            </DialogTitle>
          </DialogHeader>
          <p className="text-sm text-zinc-600 leading-relaxed">{message}</p>
          <Button
            onClick={() => void onLogin()}
            disabled={isLoading}
            className="w-full bg-white border-2 border-emerald-600 text-emerald-700 hover:bg-emerald-50 hover:border-emerald-700 font-semibold py-6 gap-2"
          >
            {isLoading ? (
              <Loader2 className="h-5 w-5 animate-spin" />
            ) : (
              <>
                <svg viewBox="0 0 24 24" className="h-5 w-5">
                  <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
                  <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
                  <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/>
                  <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/>
                </svg>
                {t("เข้าสู่ระบบด้วย Google", "Login with Google")}
              </>
            )}
          </Button>
          <p className="text-xs text-zinc-500">
            {t("หลังเข้าสู่ระบบ โค้ด WELCOME10 จะถูกใช้ให้อัตโนมัติ", "WELCOME10 will be applied automatically after login")}
          </p>
        </div>
      </DialogContent>
    </Dialog>
  );
}

...
export function StrainPricingGrid({ currency, onAdd }: Props) {
  const { t } = useLanguage();
  const [qtyById, setQtyById] = useState<Record<string, number>>(() =>
    Object.fromEntries(
      WHOLESALE_CATALOG.map((s) => [s.id, WHOLESALE_PUBLIC_MOQ])
    )
  );
    ...
    <section id="rfq" className="scroll-mt-24 border-b border-slate-200 bg-white py-12 sm:py-16">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-xl text-sm text-slate-600">
              {t(
                "ขั้นต่ำ 100 เมล็ดต่อสายพันธุ์ — ราคารวมอัปเดตตามจำนวนและสกุลเงินที่เลือก",
                "Minimum 100 seeds per strain — totals update with quantity and currency."
              )}
            </p>
          </div>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {WHOLESALE_CATALOG.map((strain) => {
            const qty = qtyById[strain.id] ?? WHOLESALE_PUBLIC_MOQ;
            const tier = resolveTier(qty);
            const unit = unitPrice(qty, currency);
            const total = lineTotal(qty, currency);
            return (
              <article
                key={strain.id}
                    ...
                    onChange={(e) => {
                      ...
                      setQtyById((prev) => ({
                        ...
                        [strain.id]: Number.isFinite(n)
                          ? Math.max(WHOLESALE_PUBLIC_MOQ, Math.floor(n))
                          : WHOLESALE_PUBLIC_MOQ,
                      }));
                    }}

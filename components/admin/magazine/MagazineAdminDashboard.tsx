      ...
      <div className="mx-auto max-w-6xl space-y-8 px-4 py-10">
        <div className="flex flex-col gap-4 border-b border-zinc-800/80 pb-6 sm:flex-row sm:items-end sm:justify-between">
          <div className="flex items-start gap-3">
            <div className="mt-0.5 flex h-10 w-10 items-center justify-center rounded-xl border border-zinc-700 bg-zinc-900/80">
              <Newspaper className="h-5 w-5 text-emerald-500/90" />
            </div>
            <div>
          ...
          <Card className="border-zinc-800/80 bg-zinc-900/40 shadow-none">
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-sm font-medium text-zinc-100">
                <Sparkles className="h-4 w-4 text-emerald-500/90" />
                Trending section
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              <p className="text-xs leading-relaxed text-zinc-500">
                ...
                <span className="text-zinc-400">manual_rank</span> (lower = higher).
              </p>
              {trendingLoading ? (
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  >
                    <SelectTrigger className="h-9 w-[220px] border-zinc-700 bg-zinc-900 text-zinc-100">
                      <SelectValue />
                    </SelectTrigger>
          ...
          <Card className="border-zinc-800/80 bg-zinc-900/40 shadow-none">
            ...
            <CardContent>
              <p className="text-xs text-zinc-500">
                Toggle <span className="text-zinc-400">HL</span> below — published +
        ...
        <Card className="border-zinc-800/80 bg-zinc-900/40 shadow-none">
          <CardHeader className="pb-2">
            <CardTitle className="flex items-center gap-2 text-sm font-medium text-zinc-100">
              <BarChart2 className="h-4 w-4 text-emerald-500/90" />
                  ...
                  return (
                    <li key={p.id}>
                      <div className="flex items-start justify-between gap-3 text-sm">
                        <span className="mt-0.5 w-5 shrink-0 tabular-nums text-zinc-600">
                          {i + 1}
                        </span>
                        <div className="min-w-0 flex-1">
                          <Link
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="line-clamp-2 font-medium text-zinc-200 transition hover:text-emerald-400/95"
                          >
                            {p.title}
                          </Link>
                          <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-zinc-800">
                            <div
                              className="h-full rounded-full bg-emerald-600/75 transition-all"
                              style={{ width: `${pct}%` }}
                            />
                          </div>
                        </div>
        ...
        <Card className="border-zinc-800/80 bg-zinc-900/40 shadow-none">
          <CardHeader className="flex flex-col gap-4 border-b border-zinc-800/80 pb-4 sm:flex-row sm:items-center sm:justify-between">
            <CardTitle className="flex items-center gap-2 text-base font-medium text-zinc-100">
              <BookOpen className="h-4 w-4 text-emerald-500/90" />
              Posts
            </CardTitle>
            <div className="flex flex-wrap items-end gap-3">
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    placeholder="Title…"
                    className="h-9 w-[min(100%,200px)] border-zinc-700 bg-zinc-900 pl-8 text-sm text-zinc-100 placeholder:text-zinc-600"
                  />
                </div>
              </div>
              <div className="space-y-1">
              ...
              <div className="space-y-1">
                ...
                <Select value={statusFilter} onValueChange={setStatusFilter}>
                  <SelectTrigger className="h-9 w-[130px] border-zinc-700 bg-zinc-900 text-zinc-100">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent className="border-zinc-700 bg-zinc-900 text-zinc-100">
                    <SelectItem value="all">All</SelectItem>
                ...
                <Table>
                  <TableHeader>
                    <TableRow className="border-zinc-800 hover:bg-transparent">
                      <TableHead className="w-12 text-zinc-500">HL</TableHead>
                      <TableHead className="text-zinc-400">Title</TableHead>
                      <TableHead className="hidden text-zinc-400 sm:table-cell">
                        Category
                      </TableHead>
                      <TableHead className="text-zinc-400">Status</TableHead>
                      <TableHead className="hidden tabular-nums text-zinc-400 md:table-cell">
                        Views
                      </TableHead>
                      <TableHead className="hidden tabular-nums text-zinc-400 lg:table-cell">
                        Rank
                      </TableHead>
                  ...
                  <TableBody>
                    {list.map((p) => (
                      <TableRow key={p.id} className="border-zinc-800/80">
                        <TableCell>
                          <Switch
                        ...
                        <TableCell className="max-w-[200px] font-medium text-zinc-100 sm:max-w-[280px]">
                        ...
                        <TableCell className="hidden tabular-nums text-zinc-500 md:table-cell">
                          {p.view_count ?? 0}
                        </TableCell>
                        <TableCell className="hidden tabular-nums text-zinc-500 lg:table-cell">
                          {p.manual_rank ?? 0}
                        </TableCell>

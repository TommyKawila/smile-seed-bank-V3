...
- **ไฟล์:** (read-only) `lib/clearance.ts` · `lib/product-variants-dedupe.ts` · `app/api/admin/clearance/route.ts` · `inventory/cell|sync` · `products/[id]|route` · `clearance-admin-service.ts` · `checkout-server-validate.ts`

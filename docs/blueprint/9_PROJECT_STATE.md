...
- **2026-06-12 critical fixes** — POS omni customer ids parse `pos-{id}` before `customer_profile_id` so points/total_spend writes are preserved; order-success transfer details render active `payment_settings.bank_accounts` only (no hardcoded account fallback); receipt payment service returns `promptPay` metadata for digital receipt QR.

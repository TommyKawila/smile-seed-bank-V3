...
Last PSI lock: **2026-08-08** · Boss Lab reaffirm **93** — **stop chasing Lab score** (LCP/SI orange + Unused JS/CSS accepted)

| Score | Mobile | Desktop |
|-------|--------|---------|
| Performance | **93** (Boss Aug 8 · variance 88–90 OK · DoD ≥90) | **100** *(prior lock; not retested this sprint)* |
| Accessibility | **100** | **95** |
| Best Practices | **100** | **100** |
| SEO | **100** | **100** |

**Lab notes (2026-08-08 `/`):** Mobile — FCP 1.1s · SI 3.6s · TBT 60ms · CLS **0** · LCP 3.1s (orange; TTFB-bound · accepted). Do **not** chase unused Prompt CSS / chunk **8536** JS while score ≥90. Do **not** chase Lab 93→95.
...
**Field CLS sprint shipped 2026-08-06** `2e8407b`: SSR logo · stable nav slot · checkout `cartReady` · QR min-height · `scrollbar-gutter: stable` · age-gate logo box · extended faces `optional`.

**Age-gate scroll-lock shipped 2026-08-08:** remove `html.overflow-hidden` toggle → `body` `position: fixed` + restore `scrollY` · overlay `overscroll-contain`. Keep `scrollbar-gutter: stable`. Mount timing still interaction/12s (no Lab LCP regression). Await Vercel SI `/`, `/checkout`, `/seeds` CLS P75 3–7d. Admin poor Desktop paths remain lower priority.

...
- **Cart sheet (breeder + pack copy)** — `CartSheet.tsx`: breeder logo (`breederLogoUrl` from `breeders.logo_url`) after product name; bilingual pack line `{n} seeds pack / แพคเกจ {n} เมล็ด` from `unit_label` (free gifts keep raw `unit_label`). `CartItem.breederLogoUrl` in `types/supabase.ts`; `AddToCartSchema` + add-to-cart from `product-detail-client.tsx`, `shop/page.tsx` `ProductCard`, admin `orders/create`; Zod `useCart.ts`.

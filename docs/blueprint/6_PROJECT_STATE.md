...
- **Storefront Magazine home (Step A)** — `app/(storefront)/blog/page.tsx`: light `#FFFFFF`, Playfair + Inter; `MagazineHeroCarousel` (soft bottom scrim, emerald CTA), `MagazineTrending` / `MagazineLatestGrid` / `MagazineCategoryPills` (off-white cards, zinc borders); `revalidate` 300; `Navbar` forces white bar on `/blog` for full-color logo.
- **Storefront Magazine article (Step B)** — `app/(storefront)/blog/[slug]/page.tsx`: light shell (`bg-white`, emerald headings); Prisma `blog_posts` + `tiptapJsonToHtml`; `MagazineArticleBody` light prose; `splitForSmartTieIn` + `SmartTieInStrip`; `parseArticleSegments` + `AffiliateProductCard`; `POST /api/blog/view` + `BlogViewTracker`; related cards + newsletter light cards.
...
### Homepage editorial (magazine strip)

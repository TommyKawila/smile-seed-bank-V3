...
- **Payment reminder + auto-cancel** — `lib/services/payment-reminder.ts` (`runPaymentReminders` / `processPaymentReminders`): เตือน L1–L3 (`PENDING_PAYMENT`, ไม่มี `slip_url`, `notification_level` &lt; 3) + หลัง L3 ครบ 1 ชม. → `autoCancelUnpaidOrderAfterReminders` (`CANCELLED` + `restoreVariantStockForOrderItems`, โน้ตระบบใน `reject_note`) + แจ้งปิดท้าย LINE/Email + log `PAYMENT_REMINDER` / `PAYMENT_AUTO_CANCEL`. **Cron** — `GET /api/cron/remind-payment` + `Authorization: Bearer ${CRON_SECRET}` เรียก `runPaymentReminders`; `vercel.json` schedule `0 0 * * *` (วันละครั้ง 00:00 UTC, Hobby).

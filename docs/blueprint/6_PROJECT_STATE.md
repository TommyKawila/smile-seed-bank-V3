...
### Safety Net
- **Admin cancel PENDING** — `app/admin/orders/page.tsx`: **ยกเลิกออเดอร์** → `rejectPayment` (`services/orders-service.ts`): Prisma transaction loads `order_items`, restores variant stock via `restoreVariantStockForOrderItems` (`lib/order-inventory.ts`, shared with void route), then `CANCELLED` + `reject_note` suffix re stock restore; allowed statuses `PENDING` | `AWAITING_VERIFICATION` | `PAYMENT_REJECTED` (future). Void route uses same helper for increments.
...
- **Storefront blog home (Step A)** — `app/(storefront)/blog/page.tsx`: strict white `#FFFFFF`, Playfair + Inter; H1 **Smile Seed Blog**; journal grid (`gap-8`/`gap-12`); `MagazineHeroCarousel` / `MagazineTrending` / `MagazineLatestGrid` / `MagazineCategoryPills` (cards: border `#F3F4F6`, hover shadow, 16:9 images `rounded-sm`); research posts: `VerifiedResearchBadge` + `lib/blog-research-category.ts`; `revalidate` 300; `Navbar` white bar on `/blog`.
- **Storefront article (Step B)** — `app/(storefront)/blog/[slug]/page.tsx`: light shell; featured image `aspect-video` + `rounded-sm`; category row + optional `VerifiedResearchBadge`; Prisma `blog_posts` + `tiptapJsonToHtml`; `MagazineArticleBody`; `SmartTieInStrip`; `BlogViewTracker`; related cards + newsletter.
- **Blog SEO (Step C)** — `generateMetadata`: title `| Smile Seed Blog`, `siteName` Smile Seed Blog; excerpt or 160-char body; OG/Twitter; `MagazineArticleJsonLd`; `MagazineArticleShare`; `lib/magazine-seo.ts`.
...
- **Smile Seed Blog CMS (WordPress-style)** — `/admin/magazine` list; `/admin/magazine/new` + `/admin/magazine/[id]/edit` with `MagazinePostForm` + `MagazineTiptapEditor`; JSON → `blog_posts.content`; Server Actions `app/admin/magazine/actions.ts`; **featured image** — `ImageUploadField` + bucket `magazine`; **`MagazineAdminDashboard`** light; post editor dark; customer-facing labels **Smile Seed Blog** / **Smile Seed Blog CMS** (`AdminSidebar` บล็อก Smile Seed).
...
- **Tier vs coupon (exclusive best deal)** — `lib/discount-utils.ts` `resolveExclusiveCartDiscounts` + `DEFAULT_TIERED_RULES` (≥฿2000 → 10%, ≥฿4000 → 20%); API fallback `GET /api/storefront/tiered-discounts`; coupon %/fixed on subtotal (aligned with `coupon-service`); pick path with lower merchandise total, tie → tier; `CartSummary.usePromoForOrder` / `promoSupersededByTier`; storefront checkout `promo_code_id` only when coupon wins; removed admin create-order auto-clear of promo when a tier discount applies.

...
- **Key highlights:**
  ...
  - **Admin product status** — `products.is_active`; `PATCH /api/admin/products/[id]/status` (derive via `deriveProductIsActiveForCatalog`); `ProductTableRow` shadcn `Switch` + toast; `useProducts({ includeInactive: true })` on `app/admin/products/page.tsx`; `ProductModal` product-level Switch for catalog visibility.

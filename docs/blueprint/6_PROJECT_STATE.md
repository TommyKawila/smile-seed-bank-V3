...
- **Payment reminder + auto-cancel** — `lib/services/payment-reminder.ts` (`runPaymentReminders`): สแกน `PENDING_PAYMENT` / `PENDING` / `PENDING_INFO` (ไม่มีสลิป, ยังไม่ paid) — แจ้ง L1 หลัง 2h (ไม่มี LINE/email ยังไม่ bump level), L2–L3 ตามเวลา; **24h จาก `created_at`** → `autoCancelUnpaidOrder24hStale` ใน `orders-service` (Prisma transaction, คืนสต็อก) ทุกกรณีรวมไม่มี contact; แจ้งยกเลิกทาง LINE/Email ถ้ามีเท่านั้น. `GET /api/cron/remind-payment` คืน `scanned` / `sent` / `autoCancelled`; `vercel.json` `0 0 * * *`.

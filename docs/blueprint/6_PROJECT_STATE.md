...
- **Critical storefront add-to-cart fix:** `components/storefront/ProductCard.tsx` normalizes Prisma/JSON BigInt string IDs (`product.id`, variant `id`, `breeder_id`) before `addToCart`, fixing home/related ProductCards that could reject valid products after `bigintToJson` API serialization.

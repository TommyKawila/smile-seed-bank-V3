...
- **Admin product gallery (Smile image pipeline)** — `ProductModal`: `ImageUploadField` (`uploadTarget="product"`, `variant="product"`, `toastOnSuccess={false}`); `galleryUrls` → `image_url`…`image_url_5` + `image_urls`; same `compressImageForMagazineUpload` (1200px / ~0.8MB) as Magazine; `uploadProductImage` / `POST /api/admin/products/upload` → bucket `product-images`, paths `products/optimized/…`; Manual Grid still uses `processAndUploadImages` (updated to same compressor + API).

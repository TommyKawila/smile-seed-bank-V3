...
import { getListableNewSeedsCountsByBreeder } from "@/services/product-service";

export type { StorefrontNewSeedsBreederBox };

export type NewSeedsBreederBannerAdmin = {
  id: number;
  breederId: number;
  breederName: string;
  logoUrl: string | null;
...
export type NewSeedsBreederBannerInput = {
  breederId: number;
  imageUrl?: string | null;
  titleTh?: string;
  titleEn?: string | null;
...
export async function upsertNewSeedsBreederBanner(
  input: NewSeedsBreederBannerInput
): Promise<NewSeedsBreederBannerAdmin> {
  ...
  const existing = await prisma.new_seeds_breeder_banners.findUnique({
    where: { breeder_id: breederId },
  });

  const maxSort = existing
    ? null
    : await prisma.new_seeds_breeder_banners.aggregate({ _max: { sort_order: true } });

  const row = await prisma.new_seeds_breeder_banners.upsert({
    where: { breeder_id: breederId },
    create: {
  ...
  const counts = await getListableNewSeedsCountsByBreeder();
  return {
    id: Number(row.id),
...
export async function reorderNewSeedsBreederBanners(
  orderedBreederIds: number[]
): Promise<void> {
  await prisma.$transaction(
    orderedBreederIds.map((breederId, index) =>
      prisma.new_seeds_breeder_banners.updateMany({
...
/** Active boxes: one card per breeder with listable pinned SKU counts. */
export async function getStorefrontNewSeedsBreederBoxes(): Promise<
  StorefrontNewSeedsBreederBox[]
> {
  const counts = await getListableNewSeedsCountsByBreeder();
  ...
  const boxes: StorefrontNewSeedsBreederBox[] = [];
  for (const [breederId, productCount] of counts) {

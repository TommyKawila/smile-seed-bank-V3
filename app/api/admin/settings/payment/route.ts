...
export async function GET() {
  ...
  try {
    const rows = await sql`
      SELECT bank_accounts, prompt_pay, line_id, messenger_url
...
export async function POST(req: Request) {
  ...
  const { bankAccounts, promptPay, lineId, messengerUrl } = parsed.data;

  const sql = getSql();

  try {
    await sql`
      INSERT INTO payment_settings (id, bank_accounts, prompt_pay, line_id, messenger_url, updated_at)
      VALUES (
        1,

...
/**
 * Homepage v2 — minimalist premium light shell; preview at `/page-v2`.
 * Smile Seed Blog insights use published rows from `blog_posts` via GET /api/storefront/magazine/recent.
 */
    ...
    <section className="border-b border-zinc-100 bg-white py-14 sm:py-16">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        <div className="mb-10 text-center sm:mb-12">
          ...
          <h2 className="mt-3 text-2xl font-bold tracking-tight text-emerald-950 sm:text-3xl">
            {t("งานวิจัย & เทคนิคการปลูก", "Research & cultivation")}
          </h2>
        ...
        {loading ? (
          <div className="grid gap-6 sm:grid-cols-3">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="h-[320px] animate-pulse rounded-2xl border border-zinc-100 bg-zinc-100"
              />
            ))}
          </div>
        ) : posts.length === 0 ? (
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {posts.map((p) => {
              ...
              return (
                <article
                  key={p.id}
                  className="flex flex-col overflow-hidden rounded-2xl border border-zinc-200 bg-zinc-50/50 shadow-sm transition hover:border-emerald-200 hover:shadow-md"
                >
                  <Link href={`/blog/${p.slug}`} className="relative block aspect-[16/10] overflow-hidden bg-zinc-100">
                    {img ? (
                      <Image
                        src={img}
                        alt=""
                  ...
                  <div className="flex flex-1 flex-col p-5">
                    {p.category && (
                      <span className="mb-2 inline-flex w-fit rounded-full border border-emerald-200 bg-white px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-emerald-800">
                        {p.category.name}
                      </span>
                    )}
                    <h3 className="line-clamp-2 text-lg font-semibold leading-snug text-emerald-950">
                      <Link href={`/blog/${p.slug}`} className="hover:text-emerald-800">
                        {p.title}
                      </Link>
                    </h3>
                    {p.excerpt && (

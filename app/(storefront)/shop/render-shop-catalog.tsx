...
export type ShopCatalogBootstrap = {
  breederSlugFromRoute?: string;
...
async function shopInitialProductLimit(): Promise<number> {
  ...
  return vp === "d" ? SHOP_CATALOG_DESKTOP_INITIAL : SHOP_CATALOG_MOBILE_INITIAL;
}

/** SSR catalog shell — plain resolved URL state only (no params/searchParams Promises). */
export async function renderShopCatalog({ breederSlugFromRoute, sp }: ShopCatalogBootstrap) {

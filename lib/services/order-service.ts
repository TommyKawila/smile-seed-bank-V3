...
const SLIP_UPLOAD_STATUSES = ["PENDING", "PENDING_PAYMENT"] as const;
...
export async function uploadSlip(
  ...
  try {
    ...
    >`
      SELECT o.id, o.payment_method, o.payment_status, o.status, o.slip_url,
             o.total_amount::text AS total_amount,
    ...
    if (
      !SLIP_UPLOAD_STATUSES.includes(
        (order.status ?? "") as (typeof SLIP_UPLOAD_STATUSES)[number]
      ) ||
    ...
    const updated = await sql<{ id: number }[]>`
      ...
      WHERE id = ${order.id}
        ...
        AND (slip_url IS NULL OR BTRIM(slip_url) = '')
      RETURNING id
    `;
    if (updated.length !== 1) {
